...
// ValidateURL enforces the outbound URL policy before any request is
// attempted.
func ValidateURL(raw string) (*url.URL, error) {
	...
	if host == "localhost" {
		return nil, fmt.Errorf("URL points to a disallowed host")
	}
...
// SafeClient returns an http.Client whose dial/redirect paths reject unsafe
// hosts and private IPs.
func SafeClient() *http.Client {
	return SafeClientWithResolver(&net.Resolver{}, 15*time.Second)
}

// SafeClientWithResolver mirrors SafeClient but lets callers inject a resolver
// (used by tests to simulate DNS results) and timeout.
