...
func (g *Greenhouse) Fetch(ctx context.Context, rawURL string) (ats.Posting, error) {
	board, id, ok := parseGreenhouseURL(rawURL)
	if !ok {
		return ats.Posting{}, fmt.Errorf("not a recognized greenhouse url: %s", rawURL)
	}
	...
	if err := json.Unmarshal(body, &payload); err != nil {
		return ats.Posting{}, fmt.Errorf("decode greenhouse response: %w", err)
	}

	description := htmlToText(html.UnescapeString(payload.Content))
	if description == "" {
		return ats.Posting{}, fmt.Errorf("greenhouse response contained no description")
	}
