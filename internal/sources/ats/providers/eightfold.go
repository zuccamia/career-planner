...
func NewEightfold() *Eightfold {
	return &Eightfold{client: util.SafeClient()}
}
...
func (e *Eightfold) Fetch(ctx context.Context, rawURL string) (ats.Posting, error) {
	tenant, jobID, domain, ok := parseEightfoldURL(rawURL)
	if !ok {
		return ats.Posting{}, fmt.Errorf("not a recognized eightfold url: %s", rawURL)
	}
	...
	if err := json.Unmarshal(body, &raw); err != nil {
		return ats.Posting{}, fmt.Errorf("eightfold decode: %w", err)
	}
	if strings.TrimSpace(raw.Name) == "" {
		return ats.Posting{}, fmt.Errorf("eightfold posting missing name")
	}
	...
	if description == "" {
		return ats.Posting{}, fmt.Errorf("eightfold posting contained no description")
	}
	...
	return ats.Posting{
		...
		Company:         ats.PrettifySlug(tenant),
