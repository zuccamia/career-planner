...
func NewSmartRecruiters() *SmartRecruiters {
	return &SmartRecruiters{client: util.SafeClient()}
}
...
func (s *SmartRecruiters) Fetch(ctx context.Context, rawURL string) (ats.Posting, error) {
	tenant, postingID, ok := parseSmartRecruitersURL(rawURL)
	if !ok {
		return ats.Posting{}, fmt.Errorf("not a recognized smartrecruiters url: %s", rawURL)
	}
	...
	if err := json.Unmarshal(body, &raw); err != nil {
		return ats.Posting{}, fmt.Errorf("smartrecruiters decode: %w", err)
	}
	if strings.TrimSpace(raw.Name) == "" {
		return ats.Posting{}, fmt.Errorf("smartrecruiters posting missing name")
	}
	...
	if description == "" {
		return ats.Posting{}, fmt.Errorf("smartrecruiters posting contained no description")
	}
