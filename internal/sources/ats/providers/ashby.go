...
func (a *Ashby) Fetch(ctx context.Context, rawURL string) (ats.Posting, error) {
	org, id, ok := parseAshbyURL(rawURL)
	if !ok {
		return ats.Posting{}, fmt.Errorf("not a recognized ashby url: %s", rawURL)
	}
	...
	if !ok {
		return ats.Posting{}, fmt.Errorf("ashby page missing JobPosting JSON-LD")
	}

	description := htmlToText(html.UnescapeString(jp.Description))
	if description == "" {
		return ats.Posting{}, fmt.Errorf("ashby posting contained no description")
	}
