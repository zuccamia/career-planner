...
func (l *Lever) Fetch(ctx context.Context, rawURL string) (ats.Posting, error) {
	company, id, ok := parseLeverURL(rawURL)
	if !ok {
		return ats.Posting{}, fmt.Errorf("not a recognized lever url: %s", rawURL)
	}
	...
	if err := json.Unmarshal(body, &payload); err != nil {
		return ats.Posting{}, fmt.Errorf("decode lever response: %w", err)
	}
	...
	if description == "" {
		return ats.Posting{}, fmt.Errorf("lever response contained no description")
	}
	...
	return ats.Posting{
		...
		Company:         ats.PrettifySlug(company),
