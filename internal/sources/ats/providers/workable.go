...
func NewWorkable() *Workable {
	return &Workable{client: util.SafeClient()}
}
...
func (w *Workable) Fetch(ctx context.Context, rawURL string) (ats.Posting, error) {
	tenant, code, ok := parseWorkableURL(rawURL)
	if !ok {
		return ats.Posting{}, fmt.Errorf("not a recognized workable url: %s", rawURL)
	}
...
func parseWorkableMarkdown(md, applyURL string) (ats.Posting, error) {
	p := ats.Posting{Provider: "workable", ApplyURL: applyURL}

	if m := workableTitleRE.FindStringSubmatch(md); len(m) == 2 {
		p.Title = strings.TrimSpace(m[1])
	}
	if p.Title == "" {
		return ats.Posting{}, fmt.Errorf("workable posting missing title")
	}
	...
	if p.DescriptionText == "" {
		return ats.Posting{}, fmt.Errorf("workable posting contained no description")
	}
	return p, nil
}
