...
func NewGeneric() *Generic {
	return &Generic{client: util.SafeClient()}
}

func (g *Generic) Name() string           { return "generic" }
func (g *Generic) Supports(_ string) bool { return true }

func (g *Generic) Fetch(ctx context.Context, rawURL string) (ats.Posting, error) {
	...
	if err != nil {
		return ats.Posting{}, fmt.Errorf("request job posting: %w", err)
	}
	defer resp.Body.Close()
	if loginRedirectURL(resp.Request.URL.String()) {
		return ats.Posting{}, fmt.Errorf("job posting requires sign-in or access protection; paste the raw description instead")
	}
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		return ats.Posting{}, fmt.Errorf("unexpected status %d", resp.StatusCode)
	}
	body, err := io.ReadAll(io.LimitReader(resp.Body, 2<<20))
	if err != nil {
		return ats.Posting{}, fmt.Errorf("read response body: %w", err)
	}
	...
	if text == "" {
		return ats.Posting{}, fmt.Errorf("no readable job description text found")
	}
	return ats.Posting{Provider: g.Name(), DescriptionText: text}, nil
}
