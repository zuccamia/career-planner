...
// fakeClient is a scripted llm.Client shared by candidate + dossier tests.
// lastPrompt is captured on every call so dossier tests can assert on the
// composed prompt content; candidate tests just ignore it.
...
func TestBuildTextSanitizesAndReturnsDossier(t *testing.T) {
	...
	got, err := svc.BuildDossier(context.Background(), Company{OfficialName: "Acme"}, "", Pages{})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestBuildTextErrorsWithoutClient(t *testing.T) {
	svc := &Service{}
	_, err := svc.BuildDossier(context.Background(), Company{OfficialName: "Acme"}, "", Pages{})
	if err == nil {
...
func TestBuildDossierPromptEmbedsWebsiteContent(t *testing.T) {
	...
	if _, err := svc.BuildDossier(
		context.Background(),
		Company{OfficialName: "Acme", Website: "https://acme.example"},
		"",
