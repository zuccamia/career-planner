...
import { STATIC_ROOT, isStaticHost } from './host.mjs';
...
// setLocale persists the choice and navigates so the server-rendered
// shell picks it up. On the hosted deploy a plain reload is enough — the
// Go server renders with the new cookie. On the static build every locale
// gets its own baked-in HTML file (e.g. /repo/en/dashboard.html vs
// /repo/vi/dashboard.html), so we have to swap the locale segment in the
// pathname and navigate; a reload would just re-fetch the same file.
export const setLocale = async (code) => {
  ...
  writeCookie(code);
  if (isStaticHost()) {
    const swapped = window.location.pathname.replace(
      new RegExp(`/${active}/`),
      `/${code}/`,
    );
    if (swapped !== window.location.pathname) {
      window.location.assign(swapped + window.location.search + window.location.hash);
      return;
    }
  }
...
const loadManifest = async () => {
  const manifest = await fetchJSON(`${STATIC_ROOT}i18n/manifest.json`);
...
const loadBundle = async (code) => {
  try {
    return await fetchJSON(`${STATIC_ROOT}i18n/${code}.json`);
  } catch (err) {
