...
// ResumeStructured shape extract-structured-resume-from-source emits, plus a
...
import { decodeJSONResponse, nonNegInt } from '../decode.mjs';
import { sanitizeText } from '../safety.mjs';
import { buildFormattedWithJDPersona } from '../prompts.mjs';
import { finalizeStructuredResume } from './extract-structured-resume-from-source.mjs';
...
export const parse = (raw, { base = null } = {}) => {
  ...
  const resume = finalizeStructuredResume(decoded.resume ?? {});
...
export const build = async (input, locale) => {
  ...
  return buildFormattedWithJDPersona('tailor-draft-resume', locale, jd,
