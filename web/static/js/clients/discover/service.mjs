...
import { loadATSSearchHosts as atsHosts } from '../../sources/ats/lookup.mjs';
import { buildSiteScopedQuery, isScarceEmployment, normalizeURL, filterStalePostings, filterDeadLinks, collectBrowserHits, preFilterHits, goneCache, extractPostings } from './helpers.mjs';
import { isATSLandingPage } from '../../sources/ats/lookup.mjs';
import { search as browserSearch } from '../../sources/search/client.mjs';
...
import { build as buildExpandPrompt, finalizeExpandQuery } from '../../prompt-handlers/discover/expand-query.mjs';
import { build as buildRankPrompt, finalizeRankJobs } from '../../prompt-handlers/discover/rank-jobs.mjs';
import { decodeJSONResponse } from '../../sources/llm/prompts.mjs';
...
// Tavily/Brave quota.
const searchInBrowser = async (query, req) => {
...
const searchOnServer = async (query, req) => {
  const body = await fetchJSON('/api/discover/search', {
    body: { query, profile: req.profile || {}, locale: req.locale || '' },
...
export const discoverInBrowser = async (req, { onStep = noopStep } = {}) => {
  ...
  const query = await stepped(onStep, 'expand', async () => {
    ...
    if (q.role_variants.length === 0) {
      ...
      if (headline) q.role_variants = [headline];
    }
    return q;
  });

  const hits = await stepped(onStep, 'search', async () => {
    const hitsByHost = byokSearchActive ? await searchInBrowser(query, req) : await searchOnServer(query, req);
...
export const discover = async (req, { onStep = noopStep } = {}) => {
  ...
  if (await isByokSearchActive()) {
    ...
    // honors the user's key. No expand query available here (LLM lives
    // server-side), so fall back to the raw headline as the sole role query.
    const headline = (req.profile?.headline || '').trim();
    const naiveQuery = { role_variants: headline ? [headline] : [], signal_keywords: [], broad_role: '' };
    const hitsByHost = await stepped(onStep, 'search', () => searchInBrowser(naiveQuery, req));
