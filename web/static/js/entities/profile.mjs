...
// FTS5 MATCH treats bare quotes, unbalanced parens, and bare boolean
// operators as syntax errors — strip them from LLM-authored queries.
const FTS_OPERATOR_TOKENS = /\b(AND|OR|NOT|NEAR)\b/g;
const sanitizeFtsQuery = (raw) => {
  const value = String(raw ?? '').replace(/["()]/g, ' ').replace(FTS_OPERATOR_TOKENS, ' ');
