...
// wasm extraction (workers/md-extract-client) → Markdown display → three
...
// per-flow logic lives in import-{brags,overview,resume}.mjs, each
...
import { extractMarkdown } from '../../workers/md-extract-client.mjs';
import { wireBragsExtract } from './import-brags.mjs';
import { wireOverviewExtract } from './import-overview.mjs';
import { wireTypstBuild } from './import-resume.mjs';
...
const handleFile = async (file) => {
  ...
  try {
    const result = await extractMarkdown(bytes);
    setStatus('');
    showResult(result);
  } catch (err) {
    setStatus('');
