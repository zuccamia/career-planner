...
import { isStaticHost, STATIC_ROOT } from '../host.mjs';

export const GOOGLE_CONFIG_ENDPOINT = '/oauth/google/config';
export const GOOGLE_CONFIG_STATIC_FILE = `${STATIC_ROOT}oauth-config.json`;
export const GOOGLE_TOKEN_ENDPOINT = '/oauth/google/token';
export const GOOGLE_REDIRECT_URI = `${STATIC_ROOT}oauth-callback.html`;
...
// server exposes an endpoint that renders GOOGLE_OAUTH_CLIENT_ID /
// GOOGLE_OAUTH_SCOPES from env vars. Cached after the first successful
// call. Returns { clientID, scopes }.
let googleOAuthConfigPromise = null;
export const getGoogleOAuthConfig = () => {
  if (!googleOAuthConfigPromise) {
    const url = isStaticHost() ? GOOGLE_CONFIG_STATIC_FILE : GOOGLE_CONFIG_ENDPOINT;
    googleOAuthConfigPromise = fetch(url).then(async (res) => {
      if (!res.ok) {
