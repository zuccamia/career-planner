...
// coerceFlexString accepts string / bool / number, mirroring Go's util.FlexString
...
// resolve to []. Mirrors Go's util.StringList UnmarshalJSON.
...
export const build = async (input, locale) => {
  ...
  const prompt = await buildFormatted('applications/extract-job-description', locale,
    input?.company_name ?? '',
    input?.role_title ?? '',
