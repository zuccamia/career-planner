// Ports the dossier-build half of internal/companies/service.go
// (finalizeDossier + sanitizeDossierResult + formatScrapedBlock). Produces
// the same Dossier shape the /api/companies/build-dossier endpoint returns
...
// Mirrors companies/lookup's careful handling — return the trimmed original
...
export const finalizeBuild = (generated) => ({
  status:                  'completed',
...
export const parse = (raw) => finalizeBuild(decodeJSONResponse(raw));

// Matches internal/companies/service.go ScrapedContentMaxBytes. Keeping the
...
export const build = async (input, locale) => {
  ...
  return buildFormatted('companies/build-dossier', locale,
    officialName,
    (input?.website ?? '').trim(),
    (input?.ats_url ?? '').trim(),
