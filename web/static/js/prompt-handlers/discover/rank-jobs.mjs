...
import { deriveLocationContext, normalizeURL } from '../../clients/discover/helpers.mjs';
...
// finalizeRankJobs ports FinalizeRank: URL whitelist against inputs, score
...
export const finalizeRankJobs = (out, postings, limit) => {
...
export const parse = (raw, extras) => {
  ...
  return { recommendations: finalizeRankJobs(decoded, extras?.input?.postings || [], limit) };
};

// build assembles the rank prompt. Input: {request, postings, limit} — same
// shape /prompts/discover/rank-jobs consumes.
